...
0. **_Make sure the rule you intend to add is not already implemented_**
1. **_Create a new branch from main for each new rule :_** Each new rule must be created in a new branch from main, with the rule name as branch name
...
6. **_Run the `Program` and make sure `All New Eleusis Rule Engine tests passed` is printed before creating for a pull request_**
